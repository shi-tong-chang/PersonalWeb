...
五個章節共用短距離位移與透明度的錯開進場。專案頁的文字從左、圖片從右滑入；專案選擇列使用 `data-reveal="fade"` 純淡入，避免移動選擇列影響原生焦點／捲動。星河薄霧、水晶球內外的流光／折射／浮動、技能軌道和聯繫訊號只在可見章節中保留輕微裝飾動態；離屏或背景分頁暫停，快速切換不排隊。內容預設為可見，不會因 JavaScript 失效而整頁空白。系統「減少動態效果」可即時停用進場與裝飾動畫，保留所有文字、連結與閱讀操作。
